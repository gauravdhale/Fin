...
if not stock_data.empty:
    ...
    def predict_future(data, days):
        if len(data) < 50:  # Ensure enough data points for ARIMA model
            st.warning("⚠️ Not enough data for ARIMA prediction.")
            return np.nan
        ...
        try:
            ...
            if not future_predictions.empty:
                ...
                st.warning("⚠️ No predictions generated by ARIMA model.")
                return np.nan
        except Exception as e:
            st.error(f"❌ Prediction Error: {e}")  # Display error in Streamlit
            return np.nan

    # Function to calculate error percentage
    def calculate_error(predicted, actual):
        if pd.isna(predicted) or pd.isna(actual):  # Use pd.isna() for pandas compatibility
            return np.nan
