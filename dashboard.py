...
if not stock_data.empty:
    ...
    col9.metric("1-Day Prediction", f"{one_day_pred:.2f}", delta=f"{error_1day:.2f}%")

    # 1-Week Prediction
    one_week_pred = predict_future(stock_data, 7)
    error_1week = calculate_error(one_week_pred, stock_data['Close'].iloc[-1])
    col10.metric("1-Week Prediction", f"{one_week_pred:.2f}", delta=f"{error_1week:.2f}%")

    # Error Percentage
    col11.metric("Error Percentage", f"{error_1day:.2f}%")
